...
bay_pums_heatrisk<-
  ...
  dplyr::group_by(PUMA) %>%
  dplyr::summarize(perc_elderly_or_children_refr = 
               sum(elderly_or_children_refr, na.rm= T)/sum(WGTP, na.rm = T)*100
  ) %>%
  left_join(
    bay_pumas %>%
      select(PUMACE10),
...
pums_pal <- colorNumeric(
  palette = "Purples",
  domain = bay_pums_heatrisk$perc_elderly_or_children_refr
)
```
...
leaflet() %>%
  ...
  addPolygons(
    ...
    fillColor = ~pums_pal(perc_elderly_or_children_refr),
    color = "white",
    opacity = 0.5,
    fillOpacity = 0.5,
    weight = 1,
    label = ~paste0(
      round(perc_elderly_or_children_refr), 
  ...
  addLegend(
    data = bay_pums_heatrisk,
    pal = pums_pal,
    values = ~perc_elderly_or_children_refr,
